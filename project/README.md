...
## Direction artistique — univers 2D cosy
...
prairie commune (**Constellation**) — où tous les joueurs se retrouvent :
plus de planètes séparées, plus de portails à emprunter. Le décor (arbres,
buissons, rochers, champignons, lampadaires, buissons de cristal...) est
généré par code (formes vectorielles simples peintes sur des `<canvas>`
au chargement) : aucun asset externe à télécharger en dehors du sprite du
personnage (`public/assets/sprites/character_atlas.png`).

Ajouter un élément de décor = ajouter une entrée dans `WORLD.decor` (voir
`public/game/render/WorldBuilder.js`) : type et nombre d'occurrences. Le
placement, le rendu et l'ombre portée sont génériques.
